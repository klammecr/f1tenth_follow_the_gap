...
public:
    ReactiveFollowGap() : Node("reactive_node")
    {
        ...
        m_fov = 100*M_PI/180;//90 * M_PI/180; // degrees of the field of view
    }
...
private:
    ...
    {   
        ...
        float gain = 20.0f;
        float bias = 20.0f;
        m_safety_bubble_rad = std::max(200.0f, gain * std::max(0.0f, (m_far_thresh -closest_dist)) + bias);
        oss << "Bubble Radius: " <<  m_safety_bubble_rad << std::endl;
	RCLCPP_INFO(get_logger(), oss.str()); 

        // Eliminate all points inside 'bubble' (set them to zero) 
        int min_sfty_idx = std::max(0, closest_point_idx - m_safety_bubble_rad);
        int max_sfty_idx = std::min((int)filtered_lidar.size()-1, closest_point_idx + m_safety_bubble_rad);
